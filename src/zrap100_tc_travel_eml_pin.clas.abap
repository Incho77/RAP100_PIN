"! @testing BDEF:ZRAP100_R_TravelTP_PIN
CLASS zrap100_tc_travel_eml_pin DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC
  FOR TESTING
...
CLASS zrap100_tc_travel_eml_pin IMPLEMENTATION.

  METHOD class_setup.
    " create the test doubles for the underlying CDS entities
    cds_test_environment = cl_cds_test_environment=>create_for_multiple_cds(
                      i_for_entities = VALUE #(
                        ( i_for_entity = 'zr_rap100_atravpin' ) ) ).
  ...
  METHOD create_with_action.
    ...
    MODIFY ENTITIES OF zr_rap100_atravpin
     ENTITY Travel
    ...
    SELECT * FROM zr_rap100_atravpin INTO TABLE @DATA(lt_travel). "#EC CI_NOWHERE
